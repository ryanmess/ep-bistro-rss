...
def get_wind():
    try:
        ...
        wind_label = soup.find(string="Wind Speed")
        wind_value = wind_label.find_next("td").text.strip() if wind_label else None
        return f"💨 Wind: {wind_value}" if wind_value else "💨 Wind: data unavailable"
    except Exception:
        return "💨 Wind: error"
